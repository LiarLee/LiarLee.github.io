...
- **通过在服务端查看 PID 消耗的 CPU 时间推算（三明治法）**：
    ...
    **如何计算**：
    假设最后输出：应用消耗了 150 秒 CPU 时间。
    压测报告显示：Total Requests: 300,000。
    $$ \text{单请求成本} = \frac{150}{300,000} = 0.0005 \text{ s} = 0.5 \text{ ms} $$
    $$ \text{物理极限 QPS (单核)} = \frac{1000 \text{ ms}}{0.5 \text{ ms}} = 2000 \text{ QPS} $$
...
2. 计算 并发可以提供的极限 qps 是多少？ 
   ...
   1000ms/0.19ms=5264 qps ， 2c 的机器 就是 5264 qps x 2= 10528 qps ， 这个就是机器的上限。

3. 计算 极限的并发度是多少 ？
   目前所有的2 cpu全用上， 最佳的并发度是：`10528*(0.56ms/1000)=c=5.8 ~ 6`, 就可以达到这个极限。  
