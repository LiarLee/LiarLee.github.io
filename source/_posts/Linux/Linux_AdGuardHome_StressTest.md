...
**结论：**
客户端观测的单请求总耗时为 0.56ms，扣除物理链路往返 0.37ms，剩下的 **0.19ms** (0.56 - 0.37) 是服务端处理一个缓存命中 DNS 请求（用户态切换+查表+回包）的粗略成本。由于没有扣除客户端自身的收发开销，这只是一个**上界估计**。它与后文基于极限吞吐量反推的每核服务时间预算 **0.125ms** 处于同一量级——真实的服务端单请求成本落在 0.125 ~ 0.19ms 区间内，两个独立来源的估计互为印证。
...
**代入本测试的数据**（见下方测试表）：单连接实测 2,500 QPS，并发加大后吞吐进入约 16,000 QPS 的平台，即加速比上限 $S(\infty) = \frac{16{,}000}{2{,}500} = 6.4$。由 $S(\infty) = \frac{1}{1-p}$ 可反解出等效串行占比 $1-p = \frac{2{,}500}{16{,}000} \approx 0.156$，即 $p \approx 0.84$。也就是说，这套系统无论并发加到多大，产出最多只有单连接的 6.4 倍，串行部分（内核中断、全局锁、调度器）吃掉了其余的理论增益。
...
2.  **效率拐点 (C=5 到 C=8)**：这是最值得关注的区域。随着并发数跨越核心数，效率曲线开始加速下滑。在 C=8 时，吞吐量进入约 1.6w 的峰值平台，但单核效率已跌至 81%。这说明 CPU 已经把大量精力花在了“抢地盘”（上下文切换和锁竞争）上，而不是实际的业务计算。
...
*   **物理极限点 (8 并发)**: 从 C=8 开始，产出进入物理极限平台（**16,319 QPS**），此时效率为 81.6%。表内最高点出现在 C=24（16,795.98），与 C=8 相差约 3%，在测试噪声范围内——可以认为系统从 8 并发起就已经打满，继续增加并发不再有实质产出。
*   **无效过载区 (12+ 并发)**: 吞吐量不再随并发增加而增长，始终卡在 1.6w 左右。效率随并发数成反比稀释（C=64 时仅剩 9.8%），延迟稳定在 6ms 左右，说明系统已完全饱和并进入了自我保护状态。
...
2.  **性能拐点 (11,000 - 14,000 QPS)**：曲线开始向上弯折。此时 CPU 核心开始接近满载，请求开始在内核队列中积压，导致延迟从 1ms 快速攀升至 2.5ms。这是系统告警的临界点。
3.  **饱和崩溃区 (16,000+ QPS)**：吞吐量达到 1.6w 的物理天花板后彻底走平。

**关于“M型震荡”的分析：**
注意图表后段（17,500 QPS 附近）的明显波动。当目标负载远超系统极限时，系统进入了不稳定的震荡状态。这种“双重下凹”现象通常意味着系统触发了**微观拥塞控制**：高频丢包导致队列短暂清空，随后又被瞬间填满，或者内核在处理中断风暴时出现了短暂的调度失效。

### 关键数据节点
根据上述详细数据，我们可以观察到以下关键性能节点：

*   **0 - 10,000 QPS (舒适区)**: 延迟死死压在 **0.5ms ~ 0.8ms 出头**。这是用户体验最好的区间。
*   **11,000 - 14,000 QPS (拐点区)**: 延迟开始起飞，从 1ms 一路爬升至 2.5ms 左右。此时 CPU 虽然还能处理，但内部队列开始积压。
*   **16,000+ QPS (饱和区)**: 蓝线（吞吐量）走平，红线（延迟）持续攀升至 6ms。

**有趣的发现：M型震荡**
在 17,500 QPS 的超高压下，我们观察到了吞吐量的下凹（15,093 QPS）与延迟的同步尖峰（4.827ms）。这是系统发生微观拥塞崩溃、丢包、队列清空、再重新填满的特征信号。
...
**Q: 500 QPS 的发包间隔是多大？为什么这个间隔能保证测到“纯净处理时间”？**

**A:** 这基于 QPS 与时间间隔的倒数关系：
1.  **计算公式**: $T = 1 / \lambda$。当 $\lambda (QPS) = 500$ 时，发包间隔为 $1000 \text{ms} / 500 = 2 \text{ms}$——也就是平均每 2ms 发出一个请求。
2.  **意义**: 单次请求往返约 0.56ms（与串行基线一致：链路 0.37ms + 处理 0.19ms）。2ms 的间隔留出了约 1.4ms 的绝对静默期，确保没有任何排队干扰，测得的是 **“纯净处理时间”**。
...
3.  **测试意义**: **2500 QPS** 代表了一个并发槽位在“绝对理想、无排队”状态下的 **饱和输出功率**。如果系统在 $N$ 个并发下无法达到 $N \times 2500$ 的实际产出，就说明系统在并行调度或硬件资源上遇到了瓶颈。前文扩展性测试的 C=1 行正好压线验证了这一点：`-Q 2500` 限速下单连接实际打出 2,499.99 QPS、平均延迟 0.421ms。若按“一问一答”的严格串行模型，实测延迟对应的单连接上限是 $1000 / 0.421 \approx 2{,}375$ QPS，dnsperf 靠连接内轻微的流水线重叠补足了约 5% 的差额（$\lambda W = 2499.99 \times 0.000421 \approx 1.05$，即平均略多于 1 个在途查询）——2,500 QPS 正好压在单连接能力的边缘。
...
1.  **计算打满系统所需的最小连接数**: 
    ...
    - 已知单连接的实测发包能力 $\approx 2,500$ QPS（扩展性测试 C=1 行：2,499.99 QPS @ 0.421ms）。
    - 要把系统压到极限，理论上至少需要 $16{,}000 / 2{,}500 = 6.4$ 个连接。（这个 6.4 同时也是阿姆达尔定律反解出的加速比上限 $S(\infty)$，两个视角殊途同归。）
2.  **选择 20 的逻辑**:
    - **20 是最小连接数的 ~3 倍**：$20 / 6.4 \approx 3.1$，确保服务器的请求队列始终是满的（Full Pipe）。
    - **20 处于稳定产出区间**：在扩展性测试中，C=20 时实测 16,069.78 QPS，产出已与 C=8 以来的峰值平台持平。
...
公式： $L = \lambda \times W$（系统内平均请求数 = 到达率 × 单请求平均停留时间）。使用时的关键约束是：$\lambda$ 和 $W$ 必须取自**同一个稳定工况**，不能一个取饱和区、一个取空载区。

- **读法一：饱和时系统内的在途请求数。** 取峰值工况的实测值（16,319 QPS、平均延迟 5.142ms）：
  $$ L = 16{,}319 \times 0.005142\,\text{s} \approx 84 $$
  即系统打满时，内部常驻约 84 个在途请求。这也说明 dnsperf 的 `-c` 并发数并不等于系统内的在途请求数——过载时每个 client 连接名下同时挂着约 10 个未完成查询（84 ÷ 8 ≈ 10），排队深度主要积累在服务端。
- **读法二：正在被 CPU 服务的请求数。** 把 $W$ 取为纯服务时间 0.125ms（下一节反推所得）：
  $$ L_{\text{in service}} = 16{,}000 \times 0.000125\,\text{s} = 2 $$
  正在 CPU 上被处理的请求数恰好等于核数 2——两颗核心全部跑满，这就是“物理极限”在微观上的含义。

**2. 物理极限推算**
$$ \text{单核极限} = \frac{16,000}{2} = 8,000 \text{ QPS} $$
$$ \text{单次处理时间预算} = \frac{1 \text{s}}{8,000} = 0.125 \text{ ms} $$

这意味着，在当前 CPU 主频下，系统（应用 + 内核网络栈）处理一个完整 DNS 请求的每核成本约为 0.125ms，与基线差值法估出的 0.19ms 上界处于同一量级。
...
- **通过抓包的时间来推算**：利用 `tcpdump` 或 Wireshark 分析报文时间戳。
    ...
    **计算逻辑**：
    1.  **记录 T1 (握手/请求)**：对于 TCP，记录三次握手完成（客户端发送 ACK）并随之发送请求报文的时间戳；对于 UDP，记录请求报文发出的时间戳。
    2.  **记录 T2 (响应)**：记录收到服务端首个响应报文的时间戳。
    3.  **估算**：$\text{服务端处理时间} \approx (T_2 - T_1) - \text{网络RTT}$。
    *(注：网络 RTT 可通过 TCP 握手的前两个包 SYN/SYN-ACK 的时间差获得，或直接参考 Ping 值)*。

- **通过在服务端查看 PID 消耗的 CPU 时间推算（三明治法）**：
    针对 5 分钟（300秒）的长测试，为了获得最精准的“物理极限”推算，单纯的人眼看是不够的。我们需要数据的精确累积值。
    ...
    **操作脚本**：
    ```bash
    # 1. 设置变量
    TARGET_PID=12345  # <--- 改这里 (AdGuardHome PID)

    # 2. 获取开始前的 CPU 累计秒数 (T1)
    # --no-headers 去掉表头，方便后续计算
    T1=$(ps -p $TARGET_PID -o times --no-headers | tr -d ' ')
    echo "开始时刻 CPU 累计时间: $T1 秒"
    ...
    # 4. 获取结束后的 CPU 累计秒数 (T2)
    T2=$(ps -p $TARGET_PID -o times --no-headers | tr -d ' ')
    echo "结束时刻 CPU 累计时间: $T2 秒"

    # 5. 自动结算
    DELTA=$((T2 - T1))
    echo "--------------------------------"
    echo "在这 5 分钟内，应用消耗了: $DELTA 秒 CPU 时间"
    ```

    **如何计算**：
    假设一次 5 分钟的饱和压测（≈16,000 QPS）共完成 $300 \times 16{,}000 = 4{,}800{,}000$ 个请求，三明治法测得 AdGuardHome 进程自身消耗了 400 秒 CPU 时间：
    $$ \text{单请求应用层成本} = \frac{400}{4{,}800{,}000} \approx 0.000083 \text{ s} = 0.083 \text{ ms} $$
    $$ \text{应用层单核极限} = \frac{1000 \text{ ms}}{0.083 \text{ ms}} \approx 12{,}000 \text{ QPS} $$

    注意这个口径只统计 AdGuardHome **进程自身**（用户态+内核态）的 CPU 消耗，不含宿主机内核协议栈、软中断和虚拟化开销。而按系统总吞吐反推的每核总成本是 0.125ms，差额 $0.125 - 0.083 \approx 0.04$ms 就是内核网络栈等系统侧的每请求成本。按总成本验算：$2 \times \frac{1\,\text{s}}{0.000125\,\text{s}} = 16{,}000$ QPS，与实测平台吻合。

- **在 Kubernetes 环境中推算**：通过 Pod 的 CPU 调度指标推算。
    在 Prometheus / Grafana 中的黄金查询语句：
    你应该使用 `rate()` 函数，且时间窗口（Range Vector）最好等于或略大于你的压测时长。
    ...
    **这个公式的物理含义是**：
    1.  **取终点**：测试结束时的 CPU 累计使用时间（比如累计用了 100秒）。
    2.  **取起点**：测试开始时的 CPU 累计使用时间（比如累计用了 82秒）。
    3.  **算增量**：$100 - 82 = 18$ 秒（这 2 分钟内，CPU 这种资源一共被消耗了 18 秒）。
    4.  **除以时长**：$18s / 120s = 0.15$。
    5.  **单位换算**：$0.15 \times 1000 = 150m$。

    （严格地说，`rate()` 是对窗口内所有样本做线性回归并外推，只有窗口内恰好两个样本时才严格等于 (终点−起点)/窗口，但物理含义一致。）
...
| **Scalability** | 8 | 20,000 | 16,319.01 | 5.142 | 进入峰值平台 |
...
| **Saturation** | 20 | 25,000 | 16,106.64 | 6.025 | 深度饱和 |
...
*   **黄区 (亚健康)**: **10,000 - 14,000 QPS**
    *   延迟 1ms - 2.5ms。开始出现排队，虽然可用，但建议扩容。
*   **红区 (不可用)**: **> 14,000 QPS**
    *   延迟超过 2.5ms 并持续恶化；当负载超过约 17,000 QPS 后，延迟 > 4ms 且伴随抖动，吞吐被 16,000 的天花板锁死。
...
# 策略: 加密采样的并发梯度 (含非 2^n 的点)，目标 QPS = 并发 * 2500
echo ">>> Phase 1: Scalability Test (Dense Gradient)"
UNIT_QPS=2500
...
# ================= 阶段 2: 饱和度 (Capacity) =================
# 策略: 固定 20 并发 (约为打满系统所需最小连接数 6.4 的 3 倍)，线性增加 QPS
echo ">>> Phase 2: Capacity Test (Finding the Knee)"
FIXED_CONC=20
# 与正文饱和度表格一致的 23 个扫描点: 500 起步, 2k~10k 步长 2k, 11k~18k 步长 0.5k, 再加 20k/25k
CAPACITY_QPS=(500 $(seq 2000 2000 10000) $(seq 11000 500 18000) 20000 25000)
...
这部分是我自己测试自己学习成果的记录， 回来复盘一下。  2025-12-30 01:19
...
1. 计算 压测客户端 cpu 可以发出串行大约多少qps请求？ 
  
   从压测的客户端可以看到， 轻负载请求来回花费的时间平均是 0.56ms ， 那么对于客户端来说， 串行发送请求（一问一答） 1s 可以达到：  1000ms/0.56ms ≈ 1786 qps 左右。 
    
2. 计算 并发可以提供的极限 qps 是多少？ 

   从服务器的cpu处理能力来计算， 0.56ms - 0.37ms = 0.19ms ， 这大概就是cpu处理一个dns请求的时间，但是这个是粗略估计的， 不太完全准确， 可以用来估算性能极限。 
   1000ms/0.19ms ≈ 5263 qps ， 2c 的机器 就是 5263 qps x 2 ≈ 10526 qps ， 这个数值是机器上限的一个保守估计。

3. 计算 极限的并发度是多少 ？ 
   目前所有的2 cpu全用上， 按一问一答的串行模型估算最佳并发度是：`10526*(0.56ms/1000)=c≈5.9 ~ 6`, 就可以达到这个极限。  
   实测也基本吻合： 扩展性测试中 C=5 就已经打出 12,391 QPS， 超过了这个估算值。

回来再看， 这个估算比实测的峰值（约 16,300 QPS）低了大约 35% ， 原因有两个： 一是 0.56ms 的串行采样里包含了客户端自身的收发开销， 高估了单请求成本； 二是串行估算默认“每核同一时刻只能处理一个请求”， 忽略了流水线并行的收益。 它真正的价值是给出一个保守下界， 并且和饱和度实测对得上： 把负载控制在这个估算值以内（约 10,500 QPS）， 单请求平均延迟可以稳定压在 1ms 上下（实测 10,000 QPS 时 0.824ms、11,000 QPS 时 1.109ms）； 而若要把延迟维持在 0.5ms 量级， 负载需要控制在 4,000 ~ 6,000 QPS 以内（实测 4,000 QPS 时 0.507ms、6,000 QPS 时 0.592ms）。
